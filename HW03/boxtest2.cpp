...
void test_class_Box_data_ctor(Tester & t)
{
    ...
    Box con0(2, 2);  // This only needs to compile
    t.test(true, "Ctor from data, two parameters");

    // Construct from data #1
    const Box con1(3,7,Box::HOLLOW);
    t.test(true, "Ctor from data, three parameter");

    // Check constructed from data #1 type
    s1 = con1.type();
    t.test(s1 == "Hollow", "Ctor from data, type");

    // Check constructed from data #1 width
    i1 = con1.getWidth();
    t.test(i1 == 3, "Ctor from data, width");

    // Check constructed from data #1 height
    i1 = con1.getHeight();
    t.test(i1 == 7, "Ctor from data, height");

    // Copy Construct
    const Box ccon1(con1);
    t.test(true, "Copy Ctor");

    // Check constructed from data #1 type
    s1 = ccon1.type();
    t.test(s1 == "Hollow", "Copy Ctor, type");

    // Check constructed from data #1 width
    i1 = ccon1.getWidth();
    t.test(i1 == 3, "Copy Ctor, width");

    // Check constructed from data #1 height
    i1 = ccon1.getHeight();
    t.test(i1 == 7, "Copy Ctor, height");

    // Construct from data #2 (const)
    const Box con2(4,2,Box::FILLED);

    // Check constructed from data #2 type
    s1 = con2.type();
    t.test(s1 == "Filled", "Ctor from data, const, type");

    // Check constructed from data #2 height
    i1 = con2.getHeight();
    t.test(i1 == 2, "Ctor from data, const, height");

    // Check constructed from data #2 width
    i1 = con2.getWidth();
    t.test(i1 == 4, "Ctor from data, const, width");

    // Construct from data #2 (const)
    const Box con3(2,2,Box::CHECKERED);

    // Check constructed from data #2 type
    s1 = con3.type();
    std::cout << s1 << std::endl;
    t.test(s1 == "Checkered", "Ctor from data, const, type");
}
...
{
    std::cout << "Test Suite: class Box, get and set functions" << std::endl;
    ...
    Box con1(4,5);
    const Box & con1r(con1);
    Box con2(6,7,Box::FILLED);
...
void test_class_Box_print(Tester & t)
{
    ...
    Box con7(5,3,Box::CHECKERED);

    // Check print #7
    os.str(""); //reset output holder
    os << con7;
    t.test(os.str() == "x x x\n x x \nx x x\n", "print 5x3 checkered box");
}

// test_class_Box_howmany
// Test suite for class Box, HowMany()
// Pre: None.
// Post:
//     Pass/fail status of tests have been registered with t.
//     Appropriate messages have been printed to cout.
// Does not throw (No-Throw Guarantee)
void test_class_Box_HowMany(Tester & t)
{
    std::cout << "Test Suite: class Box, howMany()" << std::endl;
    t.test(Box::howMany()==0, "HowMany(), none made yet");

    // Test #1
    Box b1;
    t.test(Box::howMany()==1, "howMany(), one made");

    {
    const Box arr[20];
    t.test(arr[0].howMany()==21, "howMany(), array of 20 more made");
    }

    t.test(Box::howMany()==1, "howMany(), back to just one made");

    {
    Box b2(b1);
    t.test(Box::howMany()==2, "howMany(), copy made, so two exist now");
    Box b3(2,5);
    t.test(Box::howMany()==3, "howMany(), made one with 2 paramater constructor, so 3 exist now");
    Box b4(3,5,Box::CHECKERED);
    t.test(Box::howMany()==4, "howMany(), made one with 3 paramater constructor, so 4 exist now");
    }

    t.test(Box::howMany()==1, "howMany(), once again, back to just one made");
